...
import ViewQuizStatsModal from "../../components/quiz/QuizStats";
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        ...
        setError(err.response?.data?.message || "Cannot load lesson data.");
      } finally {
        setLoading(false);
      }
    };
                    ...
                    <Row>
                      {quizzes.map((q) => (
                        <Col md={6} key={q.id} className="mb-2">
                          <Card className="h-100 border-0 bg-warning bg-opacity-10 mb-2">
                            <Card.Body className="d-flex align-items-center">
