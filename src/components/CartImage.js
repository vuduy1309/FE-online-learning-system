...
const CartImage = ({
  ...
  const getImageUrl = (imageUrl) => {
    // If there's no image URL, return the fallback
    if (!imageUrl) return fallbackSrc;
    
    // If it's a placeholder URL (/api/placeholder/...), return as is
    ...
    // Otherwise, add a slash and then append to the base URL
    return `${API_BASE_URL}/${imageUrl}`;
  };

  // Handle the image error by setting the error state to true
